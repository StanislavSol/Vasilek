...
Процедура ПечатьЗаказаПоставщику(ТабДок, Ссылка) Экспорт
	//{{_КОНСТРУКТОР_ПЕЧАТИ(ПечатьЗаказаПоставщику)
	Макет = Документы.ЗаказТоваров.ПолучитьМакет("ПечатьЗаказаПоставщику");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаказТоваров.Дата,
	|	ЗаказТоваров.Договор,
	|	ЗаказТоваров.Комментарий,
	|	ЗаказТоваров.Контрагент,
	|	ЗаказТоваров.Номер,
	|	ЗаказТоваров.СуммаДокумента,
	|	ЗаказТоваров.ТабНоменклатура.(
	|		НомерСтроки,
	|		Номенклатура,
	|		Количество,
	|		Цена,
